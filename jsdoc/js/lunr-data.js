window.lunrData = {
  "index": {
    "version": "1.0.0",
    "fields": [
      {
        "name": "longname",
        "boost": 1000
      },
      {
        "name": "name",
        "boost": 500
      },
      {
        "name": "tags",
        "boost": 300
      },
      {
        "name": "kind",
        "boost": 110
      },
      {
        "name": "title",
        "boost": 100
      },
      {
        "name": "summary",
        "boost": 70
      },
      {
        "name": "description",
        "boost": 50
      },
      {
        "name": "body",
        "boost": 1
      }
    ],
    "ref": "id",
    "tokenizer": "default",
    "documentStore": {
      "store": {
        "index.html": [
          "augmented.j",
          "develop",
          "index",
          "next",
          "present",
          "readm",
          "simpli"
        ],
        "global.html": [
          "document",
          "global"
        ],
        "list_class.html": [
          "class",
          "document",
          "list",
          "list:class"
        ],
        "History.html": [
          "back",
          "backbone.j",
          "base",
          "browser",
          "class",
          "cross",
          "fall",
          "fragment",
          "handl",
          "histori",
          "ie",
          "inspir",
          "manag",
          "natch",
          "old",
          "onhashchang",
          "poll",
          "pushstat",
          "real",
          "support",
          "thank",
          "url"
        ],
        "Router.html": [
          "action",
          "class",
          "constructor",
          "creat",
          "event",
          "faux",
          "fire",
          "flag",
          "hash",
          "map",
          "match",
          "new",
          "on",
          "pass",
          "rout",
          "router",
          "set",
          "static",
          "support",
          "transit",
          "url",
          "well"
        ],
        "Router.html#view": [
          "member",
          "router#view",
          "view"
        ],
        "Router.html#.parseQuery": [
          "function",
          "lt;static&gt",
          "object",
          "pars",
          "parsequeri",
          "queri",
          "querystr",
          "return",
          "router.parsequeri",
          "string"
        ],
        "Router.html#loadView": [
          "call",
          "cleanup",
          "function",
          "last",
          "load",
          "loadview",
          "lt;async&gt",
          "remov",
          "router#loadview",
          "safe",
          "view"
        ],
        "Router.html#cleanup": [
          "call",
          "cleanup",
          "function",
          "last",
          "lt;async&gt",
          "remov",
          "router#cleanup",
          "view"
        ],
        "Router.html#initialize": [
          "default",
          "empti",
          "function",
          "initi",
          "logic",
          "overrid",
          "router#initi"
        ],
        "Router.html#route": [
          "bind",
          "callback",
          "function",
          "manual",
          "name",
          "rout",
          "router#rout",
          "singl"
        ],
        "Router.html#execute": [
          "arg",
          "callback",
          "cleanup",
          "excel",
          "execut",
          "function",
          "handler",
          "paramet",
          "place",
          "post",
          "pre",
          "provid",
          "rout",
          "router#execut",
          "setup"
        ],
        "Router.html#navigate": [
          "fragment",
          "function",
          "histori",
          "navig",
          "option",
          "proxi",
          "router#navig",
          "save",
          "simpl"
        ],
        "Router.html#startHistory": [
          "api",
          "browser",
          "function",
          "histori",
          "option",
          "router#starthistori",
          "start",
          "starthistori",
          "us"
        ]
      },
      "length": 14
    },
    "tokenStore": {
      "root": {
        "docs": {},
        "a": {
          "docs": {},
          "u": {
            "docs": {},
            "g": {
              "docs": {},
              "m": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "d": {
                          "docs": {},
                          ".": {
                            "docs": {},
                            "j": {
                              "docs": {
                                "index.html": {
                                  "ref": "index.html",
                                  "tf": 214
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "c": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "Router.html": {
                        "ref": "Router.html",
                        "tf": 2.083333333333333
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "g": {
              "docs": {
                "Router.html#execute": {
                  "ref": "Router.html#execute",
                  "tf": 33.33333333333333
                }
              }
            }
          },
          "p": {
            "docs": {},
            "i": {
              "docs": {
                "Router.html#startHistory": {
                  "ref": "Router.html#startHistory",
                  "tf": 8.333333333333332
                }
              }
            }
          }
        },
        "d": {
          "docs": {},
          "e": {
            "docs": {},
            "v": {
              "docs": {},
              "e": {
                "docs": {},
                "l": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "p": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 14
                        }
                      }
                    }
                  }
                }
              }
            },
            "f": {
              "docs": {},
              "a": {
                "docs": {},
                "u": {
                  "docs": {},
                  "l": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "Router.html#initialize": {
                          "ref": "Router.html#initialize",
                          "tf": 7.142857142857142
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "c": {
              "docs": {},
              "u": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "global.html": {
                            "ref": "global.html",
                            "tf": 35
                          },
                          "list_class.html": {
                            "ref": "list_class.html",
                            "tf": 35
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "i": {
          "docs": {},
          "n": {
            "docs": {},
            "d": {
              "docs": {},
              "e": {
                "docs": {},
                "x": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 1300
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "p": {
                "docs": {},
                "i": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "History.html": {
                        "ref": "History.html",
                        "tf": 2.1739130434782608
                      }
                    }
                  }
                }
              }
            },
            "i": {
              "docs": {},
              "t": {
                "docs": {},
                "i": {
                  "docs": {
                    "Router.html#initialize": {
                      "ref": "Router.html#initialize",
                      "tf": 764.2857142857143
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {
              "History.html": {
                "ref": "History.html",
                "tf": 2.1739130434782608
              }
            }
          }
        },
        "n": {
          "docs": {},
          "e": {
            "docs": {},
            "x": {
              "docs": {},
              "t": {
                "docs": {
                  "index.html": {
                    "ref": "index.html",
                    "tf": 214
                  }
                }
              }
            },
            "w": {
              "docs": {
                "Router.html": {
                  "ref": "Router.html",
                  "tf": 2.083333333333333
                }
              }
            }
          },
          "a": {
            "docs": {},
            "t": {
              "docs": {},
              "c": {
                "docs": {},
                "h": {
                  "docs": {
                    "History.html": {
                      "ref": "History.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              }
            },
            "m": {
              "docs": {},
              "e": {
                "docs": {
                  "Router.html#route": {
                    "ref": "Router.html#route",
                    "tf": 33.33333333333333
                  }
                }
              }
            },
            "v": {
              "docs": {},
              "i": {
                "docs": {},
                "g": {
                  "docs": {
                    "Router.html#navigate": {
                      "ref": "Router.html#navigate",
                      "tf": 683.3333333333334
                    }
                  }
                }
              }
            }
          }
        },
        "p": {
          "docs": {},
          "r": {
            "docs": {},
            "e": {
              "docs": {
                "Router.html#execute": {
                  "ref": "Router.html#execute",
                  "tf": 3.8461538461538463
                }
              },
              "s": {
                "docs": {},
                "e": {
                  "docs": {},
                  "n": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "index.html": {
                          "ref": "index.html",
                          "tf": 214
                        }
                      }
                    }
                  }
                }
              }
            },
            "o": {
              "docs": {},
              "v": {
                "docs": {},
                "i": {
                  "docs": {},
                  "d": {
                    "docs": {
                      "Router.html#execute": {
                        "ref": "Router.html#execute",
                        "tf": 3.8461538461538463
                      }
                    }
                  }
                }
              },
              "x": {
                "docs": {},
                "i": {
                  "docs": {
                    "Router.html#navigate": {
                      "ref": "Router.html#navigate",
                      "tf": 8.333333333333332
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "History.html": {
                    "ref": "History.html",
                    "tf": 2.1739130434782608
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "Router.html#execute": {
                    "ref": "Router.html#execute",
                    "tf": 3.8461538461538463
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "s": {
              "docs": {},
              "h": {
                "docs": {},
                "s": {
                  "docs": {},
                  "t": {
                    "docs": {},
                    "a": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "History.html": {
                            "ref": "History.html",
                            "tf": 2.1739130434782608
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "s": {
              "docs": {},
              "s": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 2.083333333333333
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "s": {
                "docs": {
                  "Router.html#.parseQuery": {
                    "ref": "Router.html#.parseQuery",
                    "tf": 10
                  }
                },
                "e": {
                  "docs": {},
                  "q": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        "r": {
                          "docs": {},
                          "i": {
                            "docs": {
                              "Router.html#.parseQuery": {
                                "ref": "Router.html#.parseQuery",
                                "tf": 683.3333333333334
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "a": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "Router.html#execute": {
                          "ref": "Router.html#execute",
                          "tf": 3.8461538461538463
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "c": {
                "docs": {},
                "e": {
                  "docs": {
                    "Router.html#execute": {
                      "ref": "Router.html#execute",
                      "tf": 3.8461538461538463
                    }
                  }
                }
              }
            }
          }
        },
        "r": {
          "docs": {},
          "e": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {},
                "m": {
                  "docs": {
                    "index.html": {
                      "ref": "index.html",
                      "tf": 110
                    }
                  }
                }
              },
              "l": {
                "docs": {
                  "History.html": {
                    "ref": "History.html",
                    "tf": 2.1739130434782608
                  }
                }
              }
            },
            "t": {
              "docs": {},
              "u": {
                "docs": {},
                "r": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "Router.html#.parseQuery": {
                        "ref": "Router.html#.parseQuery",
                        "tf": 10
                      }
                    }
                  }
                }
              }
            },
            "m": {
              "docs": {},
              "o": {
                "docs": {},
                "v": {
                  "docs": {
                    "Router.html#loadView": {
                      "ref": "Router.html#loadView",
                      "tf": 11.11111111111111
                    },
                    "Router.html#cleanup": {
                      "ref": "Router.html#cleanup",
                      "tf": 16.666666666666664
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "u": {
              "docs": {},
              "t": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 6.25
                  },
                  "Router.html#route": {
                    "ref": "Router.html#route",
                    "tf": 708.3333333333334
                  },
                  "Router.html#execute": {
                    "ref": "Router.html#execute",
                    "tf": 11.538461538461538
                  }
                },
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "Router.html": {
                        "ref": "Router.html",
                        "tf": 1902.0833333333333
                      }
                    },
                    "#": {
                      "docs": {},
                      "v": {
                        "docs": {},
                        "i": {
                          "docs": {},
                          "e": {
                            "docs": {},
                            "w": {
                              "docs": {
                                "Router.html#view": {
                                  "ref": "Router.html#view",
                                  "tf": 1150
                                }
                              }
                            }
                          }
                        }
                      },
                      "l": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "a": {
                            "docs": {},
                            "d": {
                              "docs": {},
                              "v": {
                                "docs": {},
                                "i": {
                                  "docs": {},
                                  "e": {
                                    "docs": {},
                                    "w": {
                                      "docs": {
                                        "Router.html#loadView": {
                                          "ref": "Router.html#loadView",
                                          "tf": 1150
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "c": {
                        "docs": {},
                        "l": {
                          "docs": {},
                          "e": {
                            "docs": {},
                            "a": {
                              "docs": {},
                              "n": {
                                "docs": {},
                                "u": {
                                  "docs": {},
                                  "p": {
                                    "docs": {
                                      "Router.html#cleanup": {
                                        "ref": "Router.html#cleanup",
                                        "tf": 1150
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "i": {
                        "docs": {},
                        "n": {
                          "docs": {},
                          "i": {
                            "docs": {},
                            "t": {
                              "docs": {},
                              "i": {
                                "docs": {
                                  "Router.html#initialize": {
                                    "ref": "Router.html#initialize",
                                    "tf": 1150
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "r": {
                        "docs": {},
                        "o": {
                          "docs": {},
                          "u": {
                            "docs": {},
                            "t": {
                              "docs": {
                                "Router.html#route": {
                                  "ref": "Router.html#route",
                                  "tf": 1150
                                }
                              }
                            }
                          }
                        }
                      },
                      "e": {
                        "docs": {},
                        "x": {
                          "docs": {},
                          "e": {
                            "docs": {},
                            "c": {
                              "docs": {},
                              "u": {
                                "docs": {},
                                "t": {
                                  "docs": {
                                    "Router.html#execute": {
                                      "ref": "Router.html#execute",
                                      "tf": 1150
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "n": {
                        "docs": {},
                        "a": {
                          "docs": {},
                          "v": {
                            "docs": {},
                            "i": {
                              "docs": {},
                              "g": {
                                "docs": {
                                  "Router.html#navigate": {
                                    "ref": "Router.html#navigate",
                                    "tf": 1150
                                  }
                                }
                              }
                            }
                          }
                        }
                      },
                      "s": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "a": {
                            "docs": {},
                            "r": {
                              "docs": {},
                              "t": {
                                "docs": {},
                                "h": {
                                  "docs": {},
                                  "i": {
                                    "docs": {},
                                    "s": {
                                      "docs": {},
                                      "t": {
                                        "docs": {},
                                        "o": {
                                          "docs": {},
                                          "r": {
                                            "docs": {},
                                            "i": {
                                              "docs": {
                                                "Router.html#startHistory": {
                                                  "ref": "Router.html#startHistory",
                                                  "tf": 1150
                                                }
                                              }
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    },
                    ".": {
                      "docs": {},
                      "p": {
                        "docs": {},
                        "a": {
                          "docs": {},
                          "r": {
                            "docs": {},
                            "s": {
                              "docs": {},
                              "e": {
                                "docs": {},
                                "q": {
                                  "docs": {},
                                  "u": {
                                    "docs": {},
                                    "e": {
                                      "docs": {},
                                      "r": {
                                        "docs": {},
                                        "i": {
                                          "docs": {
                                            "Router.html#.parseQuery": {
                                              "ref": "Router.html#.parseQuery",
                                              "tf": 1150
                                            }
                                          }
                                        }
                                      }
                                    }
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "s": {
          "docs": {},
          "i": {
            "docs": {},
            "m": {
              "docs": {},
              "p": {
                "docs": {},
                "l": {
                  "docs": {
                    "Router.html#navigate": {
                      "ref": "Router.html#navigate",
                      "tf": 8.333333333333332
                    }
                  },
                  "i": {
                    "docs": {
                      "index.html": {
                        "ref": "index.html",
                        "tf": 14
                      }
                    }
                  }
                }
              }
            },
            "n": {
              "docs": {},
              "g": {
                "docs": {},
                "l": {
                  "docs": {
                    "Router.html#route": {
                      "ref": "Router.html#route",
                      "tf": 8.333333333333332
                    }
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "p": {
              "docs": {},
              "p": {
                "docs": {},
                "o": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "History.html": {
                          "ref": "History.html",
                          "tf": 2.1739130434782608
                        },
                        "Router.html": {
                          "ref": "Router.html",
                          "tf": 2.083333333333333
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "t": {
              "docs": {
                "Router.html": {
                  "ref": "Router.html",
                  "tf": 4.166666666666666
                }
              },
              "u": {
                "docs": {},
                "p": {
                  "docs": {
                    "Router.html#execute": {
                      "ref": "Router.html#execute",
                      "tf": 3.8461538461538463
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            "a": {
              "docs": {},
              "t": {
                "docs": {},
                "i": {
                  "docs": {},
                  "c": {
                    "docs": {
                      "Router.html": {
                        "ref": "Router.html",
                        "tf": 2.083333333333333
                      }
                    }
                  }
                }
              },
              "r": {
                "docs": {},
                "t": {
                  "docs": {
                    "Router.html#startHistory": {
                      "ref": "Router.html#startHistory",
                      "tf": 8.333333333333332
                    }
                  },
                  "h": {
                    "docs": {},
                    "i": {
                      "docs": {},
                      "s": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "o": {
                            "docs": {},
                            "r": {
                              "docs": {},
                              "i": {
                                "docs": {
                                  "Router.html#startHistory": {
                                    "ref": "Router.html#startHistory",
                                    "tf": 700
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "r": {
              "docs": {},
              "i": {
                "docs": {},
                "n": {
                  "docs": {},
                  "g": {
                    "docs": {
                      "Router.html#.parseQuery": {
                        "ref": "Router.html#.parseQuery",
                        "tf": 10
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "f": {
              "docs": {},
              "e": {
                "docs": {
                  "Router.html#loadView": {
                    "ref": "Router.html#loadView",
                    "tf": 5.555555555555555
                  }
                }
              }
            },
            "v": {
              "docs": {},
              "e": {
                "docs": {
                  "Router.html#navigate": {
                    "ref": "Router.html#navigate",
                    "tf": 8.333333333333332
                  }
                }
              }
            }
          }
        },
        "g": {
          "docs": {},
          "l": {
            "docs": {},
            "o": {
              "docs": {},
              "b": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "global.html": {
                        "ref": "global.html",
                        "tf": 2045
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "c": {
          "docs": {},
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "list_class.html": {
                      "ref": "list_class.html",
                      "tf": 635
                    },
                    "History.html": {
                      "ref": "History.html",
                      "tf": 110
                    },
                    "Router.html": {
                      "ref": "Router.html",
                      "tf": 110
                    }
                  }
                }
              }
            },
            "e": {
              "docs": {},
              "a": {
                "docs": {},
                "n": {
                  "docs": {},
                  "u": {
                    "docs": {},
                    "p": {
                      "docs": {
                        "Router.html#loadView": {
                          "ref": "Router.html#loadView",
                          "tf": 5.555555555555555
                        },
                        "Router.html#cleanup": {
                          "ref": "Router.html#cleanup",
                          "tf": 708.3333333333334
                        },
                        "Router.html#execute": {
                          "ref": "Router.html#execute",
                          "tf": 3.8461538461538463
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "s": {
                "docs": {},
                "s": {
                  "docs": {
                    "History.html": {
                      "ref": "History.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              }
            },
            "e": {
              "docs": {},
              "a": {
                "docs": {},
                "t": {
                  "docs": {
                    "Router.html": {
                      "ref": "Router.html",
                      "tf": 2.083333333333333
                    }
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "n": {
              "docs": {},
              "s": {
                "docs": {},
                "t": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "u": {
                      "docs": {},
                      "c": {
                        "docs": {},
                        "t": {
                          "docs": {},
                          "o": {
                            "docs": {},
                            "r": {
                              "docs": {
                                "Router.html": {
                                  "ref": "Router.html",
                                  "tf": 2.083333333333333
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "Router.html#loadView": {
                    "ref": "Router.html#loadView",
                    "tf": 5.555555555555555
                  },
                  "Router.html#cleanup": {
                    "ref": "Router.html#cleanup",
                    "tf": 8.333333333333332
                  }
                },
                "b": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "c": {
                      "docs": {},
                      "k": {
                        "docs": {
                          "Router.html#route": {
                            "ref": "Router.html#route",
                            "tf": 33.33333333333333
                          },
                          "Router.html#execute": {
                            "ref": "Router.html#execute",
                            "tf": 33.33333333333333
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "l": {
          "docs": {},
          "i": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "list_class.html": {
                    "ref": "list_class.html",
                    "tf": 110
                  }
                },
                ":": {
                  "docs": {},
                  "c": {
                    "docs": {},
                    "l": {
                      "docs": {},
                      "a": {
                        "docs": {},
                        "s": {
                          "docs": {},
                          "s": {
                            "docs": {
                              "list_class.html": {
                                "ref": "list_class.html",
                                "tf": 1300
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "t": {
            "docs": {},
            ";": {
              "docs": {},
              "s": {
                "docs": {},
                "t": {
                  "docs": {},
                  "a": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "i": {
                        "docs": {},
                        "c": {
                          "docs": {},
                          "&": {
                            "docs": {},
                            "g": {
                              "docs": {},
                              "t": {
                                "docs": {
                                  "Router.html#.parseQuery": {
                                    "ref": "Router.html#.parseQuery",
                                    "tf": 33.33333333333333
                                  }
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              },
              "a": {
                "docs": {},
                "s": {
                  "docs": {},
                  "y": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "c": {
                        "docs": {},
                        "&": {
                          "docs": {},
                          "g": {
                            "docs": {},
                            "t": {
                              "docs": {
                                "Router.html#loadView": {
                                  "ref": "Router.html#loadView",
                                  "tf": 33.33333333333333
                                },
                                "Router.html#cleanup": {
                                  "ref": "Router.html#cleanup",
                                  "tf": 50
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "a": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {
                  "Router.html#loadView": {
                    "ref": "Router.html#loadView",
                    "tf": 5.555555555555555
                  },
                  "Router.html#cleanup": {
                    "ref": "Router.html#cleanup",
                    "tf": 8.333333333333332
                  }
                }
              }
            }
          },
          "o": {
            "docs": {},
            "a": {
              "docs": {},
              "d": {
                "docs": {
                  "Router.html#loadView": {
                    "ref": "Router.html#loadView",
                    "tf": 5.555555555555555
                  }
                },
                "v": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "e": {
                      "docs": {},
                      "w": {
                        "docs": {
                          "Router.html#loadView": {
                            "ref": "Router.html#loadView",
                            "tf": 683.3333333333334
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "g": {
              "docs": {},
              "i": {
                "docs": {},
                "c": {
                  "docs": {
                    "Router.html#initialize": {
                      "ref": "Router.html#initialize",
                      "tf": 7.142857142857142
                    }
                  }
                }
              }
            }
          }
        },
        "b": {
          "docs": {},
          "a": {
            "docs": {},
            "c": {
              "docs": {},
              "k": {
                "docs": {
                  "History.html": {
                    "ref": "History.html",
                    "tf": 2.1739130434782608
                  }
                },
                "b": {
                  "docs": {},
                  "o": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "e": {
                        "docs": {},
                        ".": {
                          "docs": {},
                          "j": {
                            "docs": {
                              "History.html": {
                                "ref": "History.html",
                                "tf": 2.1739130434782608
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "e": {
                "docs": {
                  "History.html": {
                    "ref": "History.html",
                    "tf": 2.1739130434782608
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "o": {
              "docs": {},
              "w": {
                "docs": {},
                "s": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "History.html": {
                          "ref": "History.html",
                          "tf": 4.3478260869565215
                        },
                        "Router.html#startHistory": {
                          "ref": "Router.html#startHistory",
                          "tf": 8.333333333333332
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "n": {
              "docs": {},
              "d": {
                "docs": {
                  "Router.html#route": {
                    "ref": "Router.html#route",
                    "tf": 8.333333333333332
                  }
                }
              }
            }
          }
        },
        "f": {
          "docs": {},
          "a": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "History.html": {
                    "ref": "History.html",
                    "tf": 2.1739130434782608
                  }
                }
              }
            },
            "u": {
              "docs": {},
              "x": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 2.083333333333333
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "a": {
              "docs": {},
              "g": {
                "docs": {},
                "m": {
                  "docs": {},
                  "e": {
                    "docs": {},
                    "n": {
                      "docs": {},
                      "t": {
                        "docs": {
                          "History.html": {
                            "ref": "History.html",
                            "tf": 2.1739130434782608
                          },
                          "Router.html#navigate": {
                            "ref": "Router.html#navigate",
                            "tf": 41.66666666666666
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "r": {
              "docs": {},
              "e": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 2.083333333333333
                  }
                }
              }
            }
          },
          "l": {
            "docs": {},
            "a": {
              "docs": {},
              "g": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 2.083333333333333
                  }
                }
              }
            }
          },
          "u": {
            "docs": {},
            "n": {
              "docs": {},
              "c": {
                "docs": {},
                "t": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "o": {
                      "docs": {},
                      "n": {
                        "docs": {
                          "Router.html#.parseQuery": {
                            "ref": "Router.html#.parseQuery",
                            "tf": 110
                          },
                          "Router.html#loadView": {
                            "ref": "Router.html#loadView",
                            "tf": 110
                          },
                          "Router.html#cleanup": {
                            "ref": "Router.html#cleanup",
                            "tf": 110
                          },
                          "Router.html#initialize": {
                            "ref": "Router.html#initialize",
                            "tf": 117.14285714285714
                          },
                          "Router.html#route": {
                            "ref": "Router.html#route",
                            "tf": 110
                          },
                          "Router.html#execute": {
                            "ref": "Router.html#execute",
                            "tf": 110
                          },
                          "Router.html#navigate": {
                            "ref": "Router.html#navigate",
                            "tf": 110
                          },
                          "Router.html#startHistory": {
                            "ref": "Router.html#startHistory",
                            "tf": 110
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "h": {
          "docs": {},
          "a": {
            "docs": {},
            "n": {
              "docs": {},
              "d": {
                "docs": {},
                "l": {
                  "docs": {
                    "History.html": {
                      "ref": "History.html",
                      "tf": 2.1739130434782608
                    }
                  },
                  "e": {
                    "docs": {},
                    "r": {
                      "docs": {
                        "Router.html#execute": {
                          "ref": "Router.html#execute",
                          "tf": 3.8461538461538463
                        }
                      }
                    }
                  }
                }
              }
            },
            "s": {
              "docs": {},
              "h": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 2.083333333333333
                  }
                }
              }
            }
          },
          "i": {
            "docs": {},
            "s": {
              "docs": {},
              "t": {
                "docs": {},
                "o": {
                  "docs": {},
                  "r": {
                    "docs": {},
                    "i": {
                      "docs": {
                        "History.html": {
                          "ref": "History.html",
                          "tf": 1902.1739130434783
                        },
                        "Router.html#navigate": {
                          "ref": "Router.html#navigate",
                          "tf": 16.666666666666664
                        },
                        "Router.html#startHistory": {
                          "ref": "Router.html#startHistory",
                          "tf": 16.666666666666664
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "m": {
          "docs": {},
          "a": {
            "docs": {},
            "n": {
              "docs": {},
              "a": {
                "docs": {},
                "g": {
                  "docs": {
                    "History.html": {
                      "ref": "History.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              },
              "u": {
                "docs": {},
                "a": {
                  "docs": {},
                  "l": {
                    "docs": {
                      "Router.html#route": {
                        "ref": "Router.html#route",
                        "tf": 8.333333333333332
                      }
                    }
                  }
                }
              }
            },
            "p": {
              "docs": {
                "Router.html": {
                  "ref": "Router.html",
                  "tf": 2.083333333333333
                }
              }
            },
            "t": {
              "docs": {},
              "c": {
                "docs": {},
                "h": {
                  "docs": {
                    "Router.html": {
                      "ref": "Router.html",
                      "tf": 2.083333333333333
                    }
                  }
                }
              }
            }
          },
          "e": {
            "docs": {},
            "m": {
              "docs": {},
              "b": {
                "docs": {},
                "e": {
                  "docs": {},
                  "r": {
                    "docs": {
                      "Router.html#view": {
                        "ref": "Router.html#view",
                        "tf": 110
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "o": {
          "docs": {},
          "l": {
            "docs": {},
            "d": {
              "docs": {
                "History.html": {
                  "ref": "History.html",
                  "tf": 2.1739130434782608
                }
              }
            }
          },
          "n": {
            "docs": {
              "Router.html": {
                "ref": "Router.html",
                "tf": 2.083333333333333
              }
            },
            "h": {
              "docs": {},
              "a": {
                "docs": {},
                "s": {
                  "docs": {},
                  "h": {
                    "docs": {},
                    "c": {
                      "docs": {},
                      "h": {
                        "docs": {},
                        "a": {
                          "docs": {},
                          "n": {
                            "docs": {},
                            "g": {
                              "docs": {
                                "History.html": {
                                  "ref": "History.html",
                                  "tf": 2.1739130434782608
                                }
                              }
                            }
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "b": {
            "docs": {},
            "j": {
              "docs": {},
              "e": {
                "docs": {},
                "c": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "Router.html#.parseQuery": {
                        "ref": "Router.html#.parseQuery",
                        "tf": 10
                      }
                    }
                  }
                }
              }
            }
          },
          "v": {
            "docs": {},
            "e": {
              "docs": {},
              "r": {
                "docs": {},
                "r": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "d": {
                      "docs": {
                        "Router.html#initialize": {
                          "ref": "Router.html#initialize",
                          "tf": 7.142857142857142
                        }
                      }
                    }
                  }
                }
              }
            }
          },
          "p": {
            "docs": {},
            "t": {
              "docs": {},
              "i": {
                "docs": {},
                "o": {
                  "docs": {},
                  "n": {
                    "docs": {
                      "Router.html#navigate": {
                        "ref": "Router.html#navigate",
                        "tf": 33.33333333333333
                      },
                      "Router.html#startHistory": {
                        "ref": "Router.html#startHistory",
                        "tf": 50
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "t": {
          "docs": {},
          "h": {
            "docs": {},
            "a": {
              "docs": {},
              "n": {
                "docs": {},
                "k": {
                  "docs": {
                    "History.html": {
                      "ref": "History.html",
                      "tf": 2.1739130434782608
                    }
                  }
                }
              }
            }
          },
          "r": {
            "docs": {},
            "a": {
              "docs": {},
              "n": {
                "docs": {},
                "s": {
                  "docs": {},
                  "i": {
                    "docs": {},
                    "t": {
                      "docs": {
                        "Router.html": {
                          "ref": "Router.html",
                          "tf": 2.083333333333333
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "u": {
          "docs": {},
          "r": {
            "docs": {},
            "l": {
              "docs": {
                "History.html": {
                  "ref": "History.html",
                  "tf": 4.3478260869565215
                },
                "Router.html": {
                  "ref": "Router.html",
                  "tf": 2.083333333333333
                }
              }
            }
          },
          "s": {
            "docs": {
              "Router.html#startHistory": {
                "ref": "Router.html#startHistory",
                "tf": 8.333333333333332
              }
            }
          }
        },
        "e": {
          "docs": {},
          "v": {
            "docs": {},
            "e": {
              "docs": {},
              "n": {
                "docs": {},
                "t": {
                  "docs": {
                    "Router.html": {
                      "ref": "Router.html",
                      "tf": 2.083333333333333
                    }
                  }
                }
              }
            }
          },
          "m": {
            "docs": {},
            "p": {
              "docs": {},
              "t": {
                "docs": {},
                "i": {
                  "docs": {
                    "Router.html#initialize": {
                      "ref": "Router.html#initialize",
                      "tf": 7.142857142857142
                    }
                  }
                }
              }
            }
          },
          "x": {
            "docs": {},
            "c": {
              "docs": {},
              "e": {
                "docs": {},
                "l": {
                  "docs": {
                    "Router.html#execute": {
                      "ref": "Router.html#execute",
                      "tf": 3.8461538461538463
                    }
                  }
                }
              }
            },
            "e": {
              "docs": {},
              "c": {
                "docs": {},
                "u": {
                  "docs": {},
                  "t": {
                    "docs": {
                      "Router.html#execute": {
                        "ref": "Router.html#execute",
                        "tf": 687.1794871794872
                      }
                    }
                  }
                }
              }
            }
          }
        },
        "w": {
          "docs": {},
          "e": {
            "docs": {},
            "l": {
              "docs": {},
              "l": {
                "docs": {
                  "Router.html": {
                    "ref": "Router.html",
                    "tf": 2.083333333333333
                  }
                }
              }
            }
          }
        },
        "v": {
          "docs": {},
          "i": {
            "docs": {},
            "e": {
              "docs": {},
              "w": {
                "docs": {
                  "Router.html#view": {
                    "ref": "Router.html#view",
                    "tf": 750
                  },
                  "Router.html#loadView": {
                    "ref": "Router.html#loadView",
                    "tf": 44.44444444444444
                  },
                  "Router.html#cleanup": {
                    "ref": "Router.html#cleanup",
                    "tf": 8.333333333333332
                  }
                }
              }
            }
          }
        },
        "q": {
          "docs": {},
          "u": {
            "docs": {},
            "e": {
              "docs": {},
              "r": {
                "docs": {},
                "i": {
                  "docs": {
                    "Router.html#.parseQuery": {
                      "ref": "Router.html#.parseQuery",
                      "tf": 10
                    }
                  }
                },
                "y": {
                  "docs": {},
                  "s": {
                    "docs": {},
                    "t": {
                      "docs": {},
                      "r": {
                        "docs": {
                          "Router.html#.parseQuery": {
                            "ref": "Router.html#.parseQuery",
                            "tf": 33.33333333333333
                          }
                        }
                      }
                    }
                  }
                }
              }
            }
          }
        }
      },
      "length": 137
    },
    "corpusTokens": [
      "action",
      "api",
      "arg",
      "augmented.j",
      "back",
      "backbone.j",
      "base",
      "bind",
      "browser",
      "call",
      "callback",
      "class",
      "cleanup",
      "constructor",
      "creat",
      "cross",
      "default",
      "develop",
      "document",
      "empti",
      "event",
      "excel",
      "execut",
      "fall",
      "faux",
      "fire",
      "flag",
      "fragment",
      "function",
      "global",
      "handl",
      "handler",
      "hash",
      "histori",
      "ie",
      "index",
      "initi",
      "inspir",
      "last",
      "list",
      "list:class",
      "load",
      "loadview",
      "logic",
      "lt;async&gt",
      "lt;static&gt",
      "manag",
      "manual",
      "map",
      "match",
      "member",
      "name",
      "natch",
      "navig",
      "new",
      "next",
      "object",
      "old",
      "on",
      "onhashchang",
      "option",
      "overrid",
      "paramet",
      "pars",
      "parsequeri",
      "pass",
      "place",
      "poll",
      "post",
      "pre",
      "present",
      "provid",
      "proxi",
      "pushstat",
      "queri",
      "querystr",
      "readm",
      "real",
      "remov",
      "return",
      "rout",
      "router",
      "router#cleanup",
      "router#execut",
      "router#initi",
      "router#loadview",
      "router#navig",
      "router#rout",
      "router#starthistori",
      "router#view",
      "router.parsequeri",
      "safe",
      "save",
      "set",
      "setup",
      "simpl",
      "simpli",
      "singl",
      "start",
      "starthistori",
      "static",
      "string",
      "support",
      "thank",
      "transit",
      "url",
      "us",
      "view",
      "well"
    ],
    "pipeline": [
      "trimmer",
      "stopWordFilter",
      "stemmer"
    ]
  },
  "store": {
    "index.html": {
      "id": "index.html",
      "kind": "readme",
      "title": "Augmented.js Next Presentation",
      "longname": "index",
      "name": "Augmented.js Next Presentation",
      "tags": "index",
      "summary": "Augmented.js Next Presentation - Simply Development.",
      "description": "",
      "body": ""
    },
    "global.html": {
      "id": "global.html",
      "kind": "global",
      "title": "Globals",
      "longname": "global",
      "name": "Globals",
      "tags": "global",
      "summary": "All documented globals.",
      "description": "",
      "body": ""
    },
    "list_class.html": {
      "id": "list_class.html",
      "kind": "list",
      "title": "Classes",
      "longname": "list:class",
      "name": "Classes",
      "tags": "list:class",
      "summary": "All documented classes.",
      "description": "",
      "body": ""
    },
    "History.html": {
      "id": "History.html",
      "kind": "class",
      "title": "History",
      "longname": "History",
      "name": "History",
      "tags": "History",
      "summary": "",
      "description": "Handles cross-browser history management, based on either pushState and real URLs, or onhashchange and URL fragments. If the browser supports neither (old IE, natch), falls back to polling. Inspired By Backbone.js - Thank you! 😍",
      "body": ""
    },
    "Router.html": {
      "id": "Router.html",
      "kind": "class",
      "title": "Router",
      "longname": "Router",
      "name": "Router",
      "tags": "Router",
      "summary": "",
      "description": "Routers map faux-URLs to actions, and fire events when routes are matched. Creating a new one sets its routes hash, if not set statically. Supports passing routes to the constructor as well as a transition flag.",
      "body": ""
    },
    "Router.html#view": {
      "id": "Router.html#view",
      "kind": "member",
      "title": "view",
      "longname": "Router#view",
      "name": "view",
      "tags": "Router#view view",
      "summary": "",
      "description": ""
    },
    "Router.html#.parseQuery": {
      "id": "Router.html#.parseQuery",
      "kind": "function",
      "title": "&lt;static&gt; parseQuery( queryString )",
      "longname": "Router.parseQuery",
      "name": "parseQuery",
      "tags": "Router.parseQuery parseQuery",
      "summary": "",
      "description": "Parse a query string and return as an object"
    },
    "Router.html#loadView": {
      "id": "Router.html#loadView",
      "kind": "function",
      "title": "&lt;async&gt; loadView( view )",
      "longname": "Router#loadView",
      "name": "loadView",
      "tags": "Router#loadView loadView",
      "summary": "",
      "description": "Load a view safely and remove the last view by calling cleanup, then remove"
    },
    "Router.html#cleanup": {
      "id": "Router.html#cleanup",
      "kind": "function",
      "title": "&lt;async&gt; cleanup()",
      "longname": "Router#cleanup",
      "name": "cleanup",
      "tags": "Router#cleanup cleanup",
      "summary": "",
      "description": "Remove the last view by calling cleanup, then removes"
    },
    "Router.html#initialize": {
      "id": "Router.html#initialize",
      "kind": "function",
      "title": "initialize()",
      "longname": "Router#initialize",
      "name": "initialize",
      "tags": "Router#initialize initialize",
      "summary": "",
      "description": "Initialize is an empty function by default. Override it with your own initialization logic."
    },
    "Router.html#route": {
      "id": "Router.html#route",
      "kind": "function",
      "title": "route( route, name, callback )",
      "longname": "Router#route",
      "name": "route",
      "tags": "Router#route route",
      "summary": "",
      "description": "Manually bind a single named route to a callback."
    },
    "Router.html#execute": {
      "id": "Router.html#execute",
      "kind": "function",
      "title": "execute( callback, args )",
      "longname": "Router#execute",
      "name": "execute",
      "tags": "Router#execute execute",
      "summary": "",
      "description": "Execute a route handler with the provided parameters. This is an excellent place to do pre-route setup or post-route cleanup."
    },
    "Router.html#navigate": {
      "id": "Router.html#navigate",
      "kind": "function",
      "title": "navigate( fragment, options )",
      "longname": "Router#navigate",
      "name": "navigate",
      "tags": "Router#navigate navigate",
      "summary": "",
      "description": "Simple proxy to history to save a fragment into the history."
    },
    "Router.html#startHistory": {
      "id": "Router.html#startHistory",
      "kind": "function",
      "title": "startHistory( options )",
      "longname": "Router#startHistory",
      "name": "startHistory",
      "tags": "Router#startHistory startHistory",
      "summary": "",
      "description": "Start the history using browser History API"
    }
  }
};